...
module.exports = class Util {
  ...
  if(length < 6){
    length = 6
    console.error("[SERVER] captcha key length must be at least 5, length set to 5.")}
    else if(length > 20){
      length = 20
      console.error("[SERVER] captcha key length must be at bigger 20, length set to 20.")}
  let result = '';
