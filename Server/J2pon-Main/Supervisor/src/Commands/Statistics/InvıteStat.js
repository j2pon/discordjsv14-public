...
module.exports = {
    ...
    onCommand: async function (client, message, args) {
      ...
      if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && !kanallar.includes(message.channel.name)) return message.reply({ content: `${kanallar.map(x => `${client.channels.cache.find(chan => chan.name == x)}`)} kanallarında kullanabilirsiniz.`}).then((e) => setTimeout(() => { e.delete(); }, 10000)); 
        ...
        const davetettigim = invMember ? invMember.filter(byj2pon => message.guild.members.cache.get(byj2pon.userID)).slice(0, 10).map((byj2pon, index) => message.guild.members.cache.get(byj2pon.userID)).join(", ") : "Veri Yok"
        ...
        .setDescription(`${member} isimli üye toplam **${total}** (Bonus: \` +${bonus} \`) davete sahip. (**${regular}** giren, **${leave}** ayrılmış, **${fake}** fake, **${daily}** günlük, **${weekly}** haftalık)
