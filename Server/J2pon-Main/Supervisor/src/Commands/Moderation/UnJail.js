...
module.exports = {
    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && 
        !j2ponm.JailHammer.some(x => message.member.roles.cache.has(x)) &&
    ...
    return }
    if (!j2ponm.JailedRoles.some(x => member.roles.cache.has(x))) 
    {
    message.react(`${client.emoji("server_carpi")}`)
    ...
    const data = await penals.findOne({ userID: member.user.id, guildID: message.guild.id, type: "Jail", active: true });
    ...
    // Cezalı rollerini kaldır
    await member.roles.remove(j2ponm.JailedRoles).catch(() => {});
    
    // Kayıtsız rolünü ver (Booster varsa onu koru)
    if (member.roles.cache.has(j2ponm.BoosterRole)) {
      await member.roles.set([j2ponm.BoosterRole, ...j2ponm.UnRegisteredRoles]).catch(() => {});
    } else {
      await member.roles.set(j2ponm.UnRegisteredRoles).catch(() => {});
    }
    
    message.react(`${client.emoji("server_onay")}`)
    message.reply({ content:`${client.emoji("server_onay")} ${member.toString()} üyesinin jaili ${message.author} tarafından kaldırıldı!`}).then((e) => setTimeout(() => { e.delete(); }, 50000));
    if (j2poncik.Mainframe.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, jailiniz kaldırıldı!`}).catch(() => {});
