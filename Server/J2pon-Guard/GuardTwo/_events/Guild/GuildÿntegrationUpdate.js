...
class     integrationUpdate extends Event {
 ...
 async   onLoad(guildx) {
    ...
    if (await guvenli(j2ponnew,"server") == true){
        await guardPenaltyDB.findOneAndUpdate({guildID:guild.id,j2ponnew:j2ponnew.id},{$push:{işlemler:{Güvenilir:true,işlem:`Entagrasyon Güncelleme`,Tarih:Date.now()}}},{upsert:true})
        if(log) return log.send({embeds:[embed.setAuthor({name:`Trustworthy ✅`, iconURL:guild.iconURL()}).setDescription(`${j2ponnew}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde entegrasyon güncelledi.`)]})
        return; // Whitelist'teyse işlemi durdur
    }
    await sik(guild,j2ponnew.id,"am")
    await ytçek(j2ponnew)
    await guardPenaltyDB.findOneAndUpdate({guildID:guild.id,j2ponnew:j2ponnew.id},{$push:{işlemler:{Güvenilir:false,işlem:`Entagrasyon Güncelleme`,Tarih:Date.now()}}},{upsert:true})
    if(log) return log.send({embeds:[embed.setAuthor({name:`Not safe ❎`, iconURL:guild.iconURL()}).setDescription(`${j2ponnew}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde entegrasyon güncellediği için rolleri alındı.`)]})
    }
 }
}
