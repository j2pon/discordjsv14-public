...
const guardSicil = require("../../../../../J2pon-Guard/Schemas/guardPenalty");
...
module.exports = {
    ...
    onCommand: async function (client, message, args, byj2ponembed) {
        try {
            ...
            const data = await guardSicil.findOne({guildID:message.guild.id,j2ponnew:user.id});
            ...
            console.error("GuardSicil command error:", error);
            message.reply({embeds:[byj2ponembed.setDescription(`Guard sicil görüntülenirken bir hata oluştu: ${error.message}`)]}).then((e) => setTimeout(() => { e.delete(); }, 5000));
        }
     },
