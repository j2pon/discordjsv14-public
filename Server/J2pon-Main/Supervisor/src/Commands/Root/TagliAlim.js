...
module.exports = {
    ...
    let msg = await message.reply({ embeds: [byj2pon], components: [row] })
    ...
    collector.on("collect", async (button) => {
      ...
      if (button.customId === "on") {
        ...
        data.tagMode = true;
        data.save();
        msg.edit({ content: `${client.emoji("server_onay")} Taglı Alım modu başarıyla **Aktif** edildi!`, embeds: [], components: [] });
      }
      if (button.customId === "off") {
        ...
        data.tagMode = false;
        data.save();
        msg.edit({ content: `${client.emoji("server_onay")} Taglı Alım modu başarıyla **Deaktif** edildi!`, embeds: [], components: [] });
      }})
