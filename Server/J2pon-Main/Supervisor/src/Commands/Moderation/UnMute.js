...
if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && !kanallar.includes(message.channel.name)) return message.reply({ content: `${kanallar.map(x => `${client.channels.cache.find(chan => chan.name == x)}`)} kanallarında kullanabilirsiniz.`}).then((e) => setTimeout(() => { e.delete(); }, 10000)); 
        ...
        return }
        if (!j2ponm.MutedRole.some(x => member.roles.cache.has(x)) && !j2ponm.VMutedRole.some(x => member.roles.cache.has(x))) 
        {
        message.react(`${client.emoji("server_carpi")}`)
        ...
        .setDisabled(!j2ponm.MutedRole.some(x => member.roles.cache.has(x)));
        ...
        .setDisabled(!j2ponm.VMutedRole.some(x => member.roles.cache.has(x)));
...
Belirtilen ${member} üyesinin ${description} kaldırmak için aşağıda ki düğmeleri kullanabilirsiniz.`)
      ...
      collector.on("collect", async (button) => {
        ...
        if (button.customId === "mute") {
          ...
          if (j2poncik.Mainframe.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından susturmanız kaldırıldı!`}).catch(() => {});
...
{ name: "Affeden", value: `${message.author}`, inline: true},
      ...
      .setFooter({ text:`${moment(Date.now()).format("LLL")}` })
       await logChannel.send({ embeds: [log1]});
          ...
          await msg.edit({ embeds: [byj2pon], components: [row] });
        }
        if (button.customId === "vmute") {
          ...
          if (j2poncik.Mainframe.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından **sesli kanallarda** olan susturmanız kaldırıldı!`}).catch(() => {});
      ...
      .setFooter({ text:`${moment(Date.now()).format("LLL")}` })
       await vlogChannel.send({ embeds: [log]});
        ...
        if (button.customId === "iptal") {
          ...
          button.reply({ content:`${client.emoji("server_onay")} Başarıyla mute işlemleri menüsü kapatıldı`, embeds: [], components: [], ephemeral: true});
        }
      })
     },
