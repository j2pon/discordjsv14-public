...
module.exports = {
    ...
    onCommand: async function (client, message, args) {
      ...
      if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && 
          ...
          !j2ponm.SponsorRoles.some(x => message.member.roles.cache.has(x))) return message.channel.send({ content:"Yeterli yetkin bulunmuyor!"}).then((e) => setTimeout(() => { e.delete(); }, 5000));
    ...
    if (cezalar.length === 0) {
      return await message.reply({ embeds: [new EmbedBuilder().setDescription( `${client.emoji("server_onay")} ${member.toString()} üyesinin sicili temiz!`)]}).then((e) => setTimeout(() => { e.delete(); }, 5000));
    }

    // Duplicate ID'leri önlemek için unique value oluştur
    const selectMenu = new ActionRowBuilder().addComponents(
      ...
      .setMinValues(1)
      .addOptions([
         ...cezalar.slice(0, 25).map((x, index) => ({
              value: `penal_${x.id}_${index}`,
              label: `${x.type} (#${x.id})`,
      ...
      cezalar.length > 25 
        ?
          new StringSelectMenuBuilder()
            ...
            .setMinValues(1)
            .addOptions([
              ...cezalar.slice(25, 50).map((x, index) => ({
                    value: `penal_${x.id}_${index + 25}`,
                    label: `${x.type} (#${x.id})`,
                    description: `${x.reason.length > 100 ? x.reason.substring(0, 98).trim() + '..' : x.reason}`,
                  }))
            ])  
