...
module.exports = {
    ...
    if (j2poncik.Mainframe.dmMessages) user.send({ content :`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **${reason}** sebebiyle **kalıcı olarak** banlandınız!`}).catch(() => {});
    ...
    message.reply({ content :`${client.emoji("server_onay")} ${member ? member.toString() : user.username} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **kalıcı olarak** banlandı! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
...
{ name: "Sebep", value: ` ${reason}`, inline: true},
      ...
      await ceza.findOneAndUpdate({ guildID: message.guild.id, userID: user.id }, { $push: { ceza: 1 } }, { upsert: true });
