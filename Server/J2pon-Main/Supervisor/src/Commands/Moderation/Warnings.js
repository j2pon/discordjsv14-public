...
module.exports = {
    ...
    onCommand: async function (client, message, args) {
      ...
      if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && !j2ponm.CMuteHammer.some(x => message.member.roles.cache.has(x))) { 
      ...
      if (!member) { 
        ...
        if (warnings.length == 0) {
            return await message.reply({ embeds: [new EmbedBuilder().setDescription( `${client.emoji("server_onay")} ${member.toString()} üyesi hiç uyarı almamış!`)]}).then((e) => setTimeout(() => { e.delete(); }, 5000));
        }
