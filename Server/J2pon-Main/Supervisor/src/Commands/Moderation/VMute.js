...
module.exports = {
    ...
    onCommand: async function (client, message, args) {
        ...
        if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && !j2ponm.VMuteHammer.some(x => message.member.roles.cache.has(x))) 
        {
        ...
        return }
        if (j2ponm.VMutedRole.some(x => member.roles.cache.has(x))) 
        {
        message.react(`${client.emoji("server_carpi")}`)
        ...
        return }
        if (j2poncik.Mainframe.voicemutelimit > 0 && vmuteLimit.has(message.author.id) && vmuteLimit.get(message.author.id) == j2poncik.Mainframe.voicemutelimit) 
        {
        message.react(`${client.emoji("server_carpi")}`)
        ...
        .addComponents(
            new StringSelectMenuBuilder()
                ...
                .addOptions([
                    { label: 'Kışkırtma, Trol ve Dalgacı Davranış', description: '5 Dakika', value: 'vmute1', emoji: emojis.server_carpi },
                    { label: 'Özel Odalara İzinsiz Giriş ve Trol', description: '1 Saat', value: 'vmute2', emoji: emojis.server_carpi },
                    { label: 'Küfür, Argo, Hakaret ve Rahatsız Edici Davranış', description: '5 Dakika', value: 'vmute3', emoji: emojis.server_carpi },
                    { label: 'Abartı, Küfür ve Taciz Kullanımı', description: '30 Dakika', value: 'vmute4', emoji: emojis.server_carpi },
                    { label: 'Dini, Irki ve Siyasi değerlere Hakaret', description: '2 Hafta', value: 'vmute5', emoji: emojis.server_carpi },
                    { label: 'Sunucu Kötüleme ve Kişisel Hakaret', description: '1 Saat', value: 'vmute6', emoji: emojis.server_carpi },
                    { label: 'Soundpad, Bass gibi Uygulama Kullanmak', description: '30 Dakika', value: 'vmute7', emoji: emojis.server_carpi },
                ]),
        );
        ...
        if (duration) {
          ...
          member.roles.add(j2ponm.VMutedRole);
          ...
          if (j2poncik.Mainframe.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar **sesli kanallarda** susturuldunuz.`}).catch(() => {});
          ...
          setTimeout(async () => {
            ...
            .setTimestamp()
            member.roles.remove(j2ponm.VMutedRole);
    ...
    await logChannel.send({ embeds: [log]});
          ...
          if (j2poncik.Mainframe.voicemutelimit > 0) {
