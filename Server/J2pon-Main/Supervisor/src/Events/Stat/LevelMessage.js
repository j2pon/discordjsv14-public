  ...
  if(message.author.bot) return;
    ...
    const xpRandom = (length) => {
      ...
      if (xpp >= gerekli) {
        ...
        await levels.updateOne({ guildID: message.guild.id, userID: message.author.id }, {$inc: { top: 1 }}, { upsert: true });
        const channel = message.guild.channels.cache.find(x=> x.name == "level_log");
        if (channel) { 
          if(channel) await channel.send({content:`:tada: **${message.author}** üyesi başarıyla \`${level + 1}\` leveline yükseldi.`}).catch(() => {});
        }
        
        try {
          const backgroundPath = path.join(__dirname, '../../../../../../Global/Images/Background4.jpg');
          const levelUp = await new canvafy.LevelUp()
            .setAvatar(message.author.avatarURL({ forceStatic: true, extension: "png" }) || message.author.defaultAvatarURL)
          ...
          try {
            ...
            message.reply({ content: `${message.author} LEVEL atladı tebrikler! \`${level + 1}\` leveline yükseldin!` }).then(async msg => {
              setTimeout(async() => {
                if(msg && msg.deletable) await msg.delete().catch(() => {});
              }, 7000);
            }).catch(() => {});
