...
class roleDeleted extends Event {
 ...
 async  onLoad(role) {
    ...
    if(!entry || !entry.executor || j2ponnew.user.bot)return;
    if ( Date.now() - entry.createdTimestamp > 5000|| await guvenli(j2ponnew,"full") == true){
        return
    }
    ...
    if(log) return log.send({embeds:[embed.setAuthor({name:`Not safe ❎`, iconURL:guild.iconURL()}).setDescription(`${j2ponnew}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${role.name}** isimli rolü sildi. Rol tekrar oluşturuldu ve işlemler başlatıldı.`)]})
    }
 }
}
