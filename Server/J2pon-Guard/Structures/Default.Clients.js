...
// AsyncEventEmitter (WebSocketShard vs) için default listener limitini yükselt,
// yapıyı bozmadan "Possible memory leak" uyarılarını engelle.
...
class Body extends Client {
    constructor(options) {
        ...
        this.owners = options.owners || ["514170385114071053"]
        ...
        process.on("unhandledRejection", (err) => {
            ...
            this._logger.log(err, "caution");
        });
        process.on("warning", (warn) => {
            if (
                warn &&
                warn.name === "DeprecationWarning" &&
                typeof warn.message === "string" &&
                warn.message.includes("ready event has been renamed to clientReady")
            ) {
                // Bu uyarıyı tamamen yut (konsola yazma)
                return;
            }
            this._logger.log(warn, "varn");
        });
    ...
    async fetchEvents(active = true) {
        ...
        dirs.forEach(dir => {
            let files = fs.readdirSync(`../../J2pon-Guard/${this._dirname}/_events/${dir}`, { encoding: "utf8" }).filter(file => file.endsWith(".js"));
            this._logger.log(`${black.bgHex('#D9A384')(this._dirname.toUpperCase())} ${files.length} events loaded in ${dir} category.`, "load");
            files.forEach(file => {
                const events = new (require(`../../J2pon-Guard/${this._dirname}/_events/${dir}/${file}`))(this);
                if(events) events.on();
            });
        });
    }
    ...
    async fetchEvents2(active = true) {
        ...
        let dirs = fs.readdirSync('./_events', { encoding: "utf8" });
        dirs.forEach(dir => {
            let files = fs.readdirSync(`../../J2pon-Guard/Guard/_events/${dir}`, { encoding: "utf8" }).filter(file => file.endsWith(".js"));
            this._logger.log(`${black.bgHex('#D9A384')(this._dirname.toUpperCase())} ${files.length} events loaded in ${dir} category.`, "load");
            files.forEach(file => {
                const events = new (require(`../../J2pon-Guard/Guard/_events/${dir}/${file}`))(this);
                if(events) events.on();
            });
        });
    }
    ...
    async fetchEvents3(active = true) {
        ...
        let dirs = fs.readdirSync('./_events', { encoding: "utf8" });
        dirs.forEach(dir => {
            let files = fs.readdirSync(`../../J2pon-Guard/GuardTwo/_events/${dir}`, { encoding: "utf8" }).filter(file => file.endsWith(".js"));
            this._logger.log(`${black.bgHex('#D9A384')(this._dirname.toUpperCase())} ${files.length} events loaded in ${dir} category.`, "load");
            files.forEach(file => {
                const events = new (require(`../../J2pon-Guard/GuardTwo/_events/${dir}/${file}`))(this);
                if(events) events.on();
            });
        });
    }
    ...
    async connect(token = this.token) {
        ...
        if(this.MongoURI) {
            await mongoose.connect(this.MongoURI).then(async (a) => {
