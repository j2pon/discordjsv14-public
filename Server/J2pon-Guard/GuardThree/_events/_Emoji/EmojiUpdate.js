...
class emojiUpdate extends Event {
 ...
 async  onLoad(oldEmoji, newEmoji) {
    ...
    if (await guvenli(j2ponnew,"emojisticker") == true){
        await guardPenaltyDB.findOneAndUpdate({guildID:guild.id,j2ponnew:j2ponnew.id},{$push:{işlemler:{Güvenilir:true,işlem:`Emoji Güncelleme (${oldEmoji.name})`,Tarih:Date.now()}}},{upsert:true})
        if(log) return log.send({embeds:[embed.setAuthor({name:`Trustworthy ✅`, iconURL:guild.iconURL()}).setDescription(`${j2ponnew}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${newEmoji.name}** isimli emojiyi güncelledi`)]})
    ...
    await guardPenaltyDB.findOneAndUpdate({guildID:guild.id,j2ponnew:j2ponnew.id},{$push:{işlemler:{Güvenilir:false,işlem:`Emoji Güncelleme (${oldEmoji.name})`,Tarih:Date.now()}}},{upsert:true})

    if(log) return log.send({embeds:[embed.setAuthor({name:`Not safe ❎`, iconURL:guild.iconURL()}).setDescription(`${j2ponnew}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${oldEmoji.name}** isimli emojiyi güncellediği için rolleri alındı ve emoji eski haline getirildi.`)]})
    }
 }
}
