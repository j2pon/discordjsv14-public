...
module.exports = {
    ...
    onCommand: async function (client, message, args) {
        ...
        if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && !j2ponm.JailHammer.some(x => message.member.roles.cache.has(x))) 
        {
        ...
        message.react(`${client.emoji("server_carpi")}`) 
        return }
        if (j2ponm.JailedRoles.some(x => member.roles.cache.has(x))) { message.channel.send({ content:"Bu üye zaten jailde!"}).then((e) => setTimeout(() => { e.delete(); }, 5000));
        message.react(`${client.emoji("server_carpi")}`) 
        return }
        ...
        message.reply({content: `Kullanıcıyı reklam'dan dolayı karantinaya atmak için ekran görüntüsü atınız.`}).then(() => {message.channel.awaitMessages({filter: (m) => m.member.id === message.author.id,max: 1,time: 120000}).then(async byj2pon => {
          if (!byj2pon.first().attachments?.first()) { 
          ...
          member.roles.cache.has(j2ponm.BoosterRole) ? member.roles.set([j2ponm.BoosterRole, j2ponm.JailedRoles[0]]) : member.roles.set(j2ponm.JailedRoles)
          ...
          penal.proofImage = byj2pon.first().attachments.first().proxyURL
          penal.save()


          message.reply({ content:`${client.emoji("server_onay")} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle jaillendi! Ceza Numarası: (\`#${penal.id}\`)`}).then((e) => setTimeout(() => { e.delete(); }, 50000));
          if (j2poncik.Mainframe.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle jaillendiniz.`, files: [byj2pon.first().attachments.first()]}).catch(() => {});
          ...
          .setImage(byj2pon.first().attachments.first().proxyURL)
            await logChannel.send({ embeds: [log]});
          }).catch(err => message.reply({ content :`${client.emoji("server_carpi")} Ekran görüntüsü atılmadığı için reklam işlemi iptal edildi.`}) + console.log(err));
          });
     },
  };