...
module.exports = {
    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && 
        !j2ponm.CMuteHammer.some(x => message.member.roles.cache.has(x)) &&
        !j2ponm.SponsorRoles.some(x => message.member.roles.cache.has(x))) 
    {
    ...
    return }
    if (j2ponm.MutedRole.some(x => member.roles.cache.has(x))) { message.channel.send({ content:"Bu üye zaten susturulmuş!"}).then((e) => setTimeout(() => { e.delete(); }, 5000));
    ...
    .addComponents(
        new StringSelectMenuBuilder()
            ...
            .addOptions([
                { label: 'Kışkırtma, Trol ve Dalgacı Davranış', description: '5 Dakika', value: 'mute1', emoji: '⚠️'},
                { label: 'Flood,Spam ve Capslock Kullanımı', description: '5 Dakika', value: 'mute2', emoji: '🔇'},
                { label: 'Metin Kanallarını Amacı Dışında Kullanmak', description: '10 Dakika', value: 'mute3', emoji: '🚫'},
                { label: 'Küfür, Argo, Hakaret ve Rahatsız Edici Davranış', description: '15 Dakika', value: 'mute4', emoji: '💢'},
                { label: 'Abartı, Küfür ve Taciz Kullanımı', description: '30 Dakika', value: 'mute5', emoji: '🔞'},
                { label: 'Dini, Irki ve Siyasi değerlere Hakaret', description: '14 Gün', value: 'mute6', emoji: '⛔'},
                { label: 'Sunucu Kötüleme ve Kişisel Hakaret', description: '1 Saat', value: 'mute7', emoji: '👎'},
             ]),
    );
    ...
    if (duration) {
      ...
      member.roles.add(j2ponm.MutedRole);
      const penal = await client.penalize(message.guild.id, member.id, 'Chat-Mute', true, message.author.id, reason, true, Math.floor(Date.now() + duration))
      message.react(`${client.emoji("server_onay")}`)
      if(ananin) ananin.delete();
      await message.channel.send({ embeds: [new EmbedBuilder().setDescription(` ${member.toString()} kullanıcısı başarıyla ** "${reason}"** sebebiyle <t:${Math.floor((Date.now() + duration) / 1000)}:R> süre boyunca yazı susturması cezası aldı. (Ceza Numarası: \`#${penal.id}\`)`)]})
      ...
      setTimeout(async () => {
        ...
        .setTimestamp()
        member.roles.remove(j2ponm.MutedRole);
 ...
 { name: "Ceza Sebebi", value: `\`\`\`fix\n${reason}\n\`\`\``, inline: false},
      ...
      if (settings.Mainframe.chatmutelimit > 0) {
        ...
        var ananin = await message.channel.send({
            embeds: [
                new EmbedBuilder()
                    .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL() })
                    .setDescription(
                        `Aşağıda bulunan menüden metin kanallarından susturmak istediğiniz ${member.toString()} için uygun olan ceza sebebini ve süresini seçiniz!`
                    ),
            ],
            components: [row],
        });
  ...
  await logChannel.send({ embeds : [log]});
    ...
    message.react(`${client.emoji("server_onay")}`)
    if(ananin) ananin.delete();
    await await message.channel.send({ embeds: [new EmbedBuilder().setDescription(` ${member.toString()} kullanıcısı başarıyla ** "${reason}"** sebebiyle <t:${Math.floor((Date.now() + duration) / 1000)}:R> süre boyunca yazı susturması cezası aldı. (Ceza Numarası: \`#${penal.id}\`)`)]})
    if (settings.Mainframe.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar susturuldunuz.`}).catch(() => {});
