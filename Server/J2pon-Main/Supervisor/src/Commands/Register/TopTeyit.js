...
module.exports = {
    ...
    let kayit = data.filter((x) => message.guild.members.cache.has(x.userID)).splice(0, 20).map((x, i) => `${x.userID === message.author.id ? `${client.sayıEmoji(i == 0 ? `1` : `${i + 1}`)} <@${x.userID}>: \` ${x.top} Kayıt \` **(Komutu Kullanan Kişi)**` : ` ${client.sayıEmoji(i == 0 ? `1` : `${i + 1}`)} <@${x.userID}>: \` ${x.top} Kayıt \``}`).join("\n"); 
