...
module.exports = {
    ...
    onCommand: async function (client, message, args) {
        ...
        if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && 
            !setup.WarnHammer.some(x => message.member.roles.cache.has(x))) {
        ...
        message.reply({ content: `${client.emoji("server_onay")} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle uyarıldı! Ceza Numarası: (\`#${penal.id}\`)`}).then((e) => setTimeout(() => { e.delete(); }, 50000));
